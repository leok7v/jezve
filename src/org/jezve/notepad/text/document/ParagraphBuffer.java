...
final class ParagraphBuffer {
    ...
    ParagraphBuffer(CharBuffer charBuffer) {
    ...
    public void replace(int start, int limit, MConstText srcText, int srcStart, int srcLimit,
        ...
        boolean separatorAtEnd =
                lastPLimit > srcStart && isParagraphBreak(srcText.at(lastPLimit - 1));
        ...
        if (limit == origLength && lastPLimit == paragraphStart) {
            ...
            while (paragraphStart < lastPLimit) {
                ...
                if (paragraphLimit < lastPLimit || separatorAtEnd) {
                    fRunArray.fRunStart[++fRunArray.fPosEnd] =
                            paragraphLimit - 1 + start - srcStart;
                }
     ...
     * <tt>style</tt>.
     */
    public boolean modifyParagraphStyles(int start, int limit, StyleModifier modifier,
            int[] damagedRange) {
